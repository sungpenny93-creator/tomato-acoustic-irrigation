...
#### 「即時訊號」與「即時推論」的頻譜圖為什麼長得不一樣

兩張是**不同函式、給不同對象看的**，本來就不一樣，不是 bug：

| | 即時訊號（上面那張大圖） | 即時推論（下面那張小圖） |
|---|---|---|
| 產生者 | `pi/spectro_render.py` | `src/spectrogram.py` |
| 給誰看 | **給人看**，判斷訊號好壞 | **給 CNN 模型吃** |
| 頻率軸 | 線性 0–500 Hz（`scipy.signal.spectrogram`） | Mel 刻度，`fmin` 照模型設定（`Config.FMIN`；超過奈奎斯特頻率會自動退回 0～sr/2） |
| 樣式 | 有座標軸、colorbar、magma 配色 | 去座標軸、壓成 224×224、另一種配色 |
| 縮放 | dB | `power_to_db`，`ref=np.max` |

判斷訊號品質請看「**即時訊號**」那張。「即時推論」那張是為了讓輸入長得跟訓練資料一致而刻意簡化的，
而且目前模型是用 250 kHz 資料訓練、SpikerBox 只有 10 kHz，推論機率**沒有參考價值**（見下方「模型與硬體取樣率不一致」）。
...
### ⚠️ 模型與硬體取樣率不一致（目前 AI 推論結果不可信）

| | Khait 資料集 / 目前的模型 | Plant SpikerBox（實際硬體） |
|---|---|---|
| 物理量 | 空氣中的**超音波氣穴聲** | 植株的**生物電位**（電訊號） |
| 感測器 | 超音波麥克風 | 電極夾（紅夾莖、黑夾土） |
| 取樣率 | 250 kHz | 韌體固定約 **10 kHz** |
| 頻段 | `FMIN=20 kHz`、`FMAX=100 kHz` | 奈奎斯特頻率只有 5 kHz，涵蓋不到上述頻段 |

用 250 kHz 超音波資料訓練的模型，餵它 10 kHz 的電位訊號，產生的頻譜圖幾乎空白，
**輸出的 noise / normal / thirsty 機率是亂猜的**。「即時訊號」卡片的波形/頻譜是真實資料，可正常使用；
「即時推論」的數字目前僅供介面展示。

要讓 AI 這塊有意義，三選一：

1. **買超音波麥克風**（如 Dodotronic Ultramic 250K/384K）→ 現有模型與 `src/config.py` 的 250 kHz 設定即可直接沿用。
2. **改做電訊號版**：用 SpikerBox 自己錄「正常 / 缺水」訊號 → 分類貼標籤 → 用符合 10 kHz 的參數重新訓練。
3. **放掉 AI**：用土壤濕度感測器 + `pi/soil_irrigation.py` 做門檻式自動澆水（程式已完成，與聲學/AI 完全獨立）。
